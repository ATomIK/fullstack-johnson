...
@app.route('/meme_generator/v1.0/save_meme', methods=['POST'])
def save_meme():
    ...
    file_name = image_path[-1]
    folder_name = image_path[-2]

    subprocess.call(["cp", "./"+folder_name+"/"+file_name, "./memes/"+file_name])


    if os.path.isfile("./memes/"+file_name):
        return jsonify({"success":True,"meme":"./memes/"+file_name})
    else:
        return jsonify({"success":False,"error":"Error saving file."})
