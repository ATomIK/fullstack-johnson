...
if(isset($_GET['generate'])){
  ...
  imagecopy($dest, $src, imagesx($dest) - $rx, imagesy($dest) - $ry, 0, 0, 16, 36);
  ...
  $mongo->insert([
    [
      "game_id" => $game_id,
      "x" => imagesx($dest) - $rx - 50,
      "y" => imagesy($dest) - $ry - 50
    ]
  ]);

  // header('Content-Type: image/png');
  imagepng($dest, "../game_images/".$game_id.".png");
  echo json_encode(["status" => true, "x" => imagesx($dest) - $rx, "y" => imagesy($dest) - $ry], JSON_PRETTY_PRINT);
...
if(isset($_GET['x']) && isset($_GET['y']) && isset($_GET['gid'])){
  ...
  echo sqrt(pow($x - $games[$gid]->x,2) + pow($y - $games[$gid]->y,2));
