...
@app.route('/meme/new', methods=['POST'])
def new_meme():
    ihatethis = {} # let's got to php ffs

    ihatethis["top_text"] = request.form["top_text"]
    ihatethis["bot_text"] = request.form["bot_text"]
    ihatethis["style_info"] = request.form["style_info"]

    nextId = client[db]["memes"].find().count()
    ihatethis["_id"] = nextId

    ihatethis["file_id"] = 5
    ihatethis["owner_id"] = 2

    createMeme = client[db]["memes"].insert(ihatethis)

    return jsonify(ihatethis)
